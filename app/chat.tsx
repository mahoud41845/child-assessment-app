...
import { useNotification } from "@/components/notification";
import { Colors } from "@/constants/colors";
import { useTranslation } from "@/context/TranslationProvider"; // استيراد الترجمة
import { chatService } from "@/services/chatService";
import { Ionicons } from "@expo/vector-icons";
import { useLocalSearchParams, useRouter } from "expo-router";
import React, { useEffect, useRef, useState } from "react";
...
export default function ChatScreen() {
  ...
  const router = useRouter();
  const { showError } = useNotification();
  const { t, isRTL } = useTranslation(); // استخدام t و isRTL
  ...
  const handleSend = async () => {
    if (!text.trim()) return showError(t("write_msg_first"));
  ...
  const TypingIndicator = () => (
    <View style={styles.typingContainer}>
      ...
      <View style={styles.typingBubble}>
        <ActivityIndicator size="small" color={Colors.textSecondary} />
        <Text style={styles.typingText}>{t("assistant_typing")}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={styles.title}>{t("smart_assistant")}</Text>
          <View style={styles.statusRow}>
            <View style={styles.onlineDot} />
            <Text style={styles.subtitle}>{t("online_now")}</Text>
          </View>
        </View>
        <TouchableOpacity style={styles.infoBtn}>
          <Ionicons
      ...
      <View style={styles.content}>
        {loading ? (
          <View style={styles.loader}>
            <ActivityIndicator size="large" color={Colors.primary} />
            <Text style={styles.loadingText}>{t("preparing_chat")}</Text>
          </View>
        ) : (
          <FlatList
            ref={flatRef}
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                <Ionicons
                  ...
                  color={Colors.border}
                />
                <Text style={styles.emptyTitle}>{t("start_chat_now")}</Text>
                <Text style={styles.emptySub}>{t("ask_me_anything")}</Text>
              </View>
            }
            ListFooterComponent={
              isTyping ? <TypingIndicator /> : <View style={{ height: 20 }} />
...
const getStyles = (isRTL: boolean) =>
  StyleSheet.create({
    ...
    userBubble: {
      backgroundColor: Colors.primary,
      borderBottomRightRadius: isRTL ? 20 : 4,
      borderBottomLeftRadius: isRTL ? 4 : 20,
    },
    botBubble: {
      backgroundColor: "#FFFFFF",
      borderBottomLeftRadius: isRTL ? 20 : 4,
      borderBottomRightRadius: isRTL ? 4 : 20,
      borderWidth: 1,
      borderColor: "#E2E8F0",
    },
    msgText: {
